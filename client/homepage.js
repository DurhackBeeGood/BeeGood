let map, infoWindow;
let dataset = JSON.parse(fs.readFileSync('../Charities.json'))['Charities'];
...
function initMap() {
    ...
    if (navigator.geolocation) {
        ...
        for (let i=0; i < dataset.length;i++){
            if (dataset[i]['Volunteering Hours Per Week']>10){
                svgMarker.fillColor = 'red'
            } else if (dataset[i]['Volunteering Hours Per Week']>5){
                svgMarker.fillColor='orange'
            } else if (dataset[i]['Volunteering Hours Per Week']>2) {
